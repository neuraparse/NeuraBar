...
final class ClipboardManager: ObservableObject {
    ...
    /// Pasteboard markers set by password managers / Keychain / browsers for
    /// secrets and throwaway items. We never capture or persist these — a 2026
    /// privacy baseline (and these would otherwise sync into iCloud/Drive in
    /// plaintext). See nspasteboard.org.
    ...
    /// Pure + testable: should we skip capturing the current pasteboard?
    static func shouldSkip(types: [NSPasteboard.PasteboardType]?) -> Bool {
        guard let types else { return false }
        return !concealedTypes.isDisjoint(with: Set(types))
    }
    ...
    func start() {
        ...
        scheduleTimer()
        // Re-evaluate cadence when Low Power Mode toggles so we back off on
        // battery. Block-based observer (ClipboardManager isn't an NSObject).
    ...
    /// Polling interval: the changeCount read is cheap, but a high-frequency
    /// wake-up keeps the process from ever idling, so we use a relaxed 1.5s
    /// and stretch to 3s in Low Power Mode. Clipboard history doesn't need
    /// sub-second latency.
    static func pollInterval(lowPower: Bool) -> TimeInterval { lowPower ? 3.0 : 1.5 }

    private func scheduleTimer() {
        timer?.invalidate()
        let interval = Self.pollInterval(lowPower: ProcessInfo.processInfo.isLowPowerModeEnabled)
    ...
    private func tick() {
        ...
        if Self.shouldSkip(types: pb.types) { return }
        guard let s = pb.string(forType: .string) else { return }
        let trimmed = s.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty, trimmed.count < 100_000 else { return }
        // Dedupe: if this exact text already exists, move it to the top with a
        // fresh timestamp so the date-sorted view agrees with array order.
        if let idx = items.firstIndex(where: { $0.text == trimmed }) {
            let existing = items.remove(at: idx)
            items.insert(ClipItem(text: existing.text, date: Date(), pinned: existing.pinned), at: 0)
            return
        }
    ...
    private func schedulePersist() {
        ...
        persistTimer = Timer.scheduledTimer(withTimeInterval: 0.6, repeats: false) { [weak self] _ in
            ...
            Persistence.saveAsync(self.items, to: self.file)
        }
    }
}
